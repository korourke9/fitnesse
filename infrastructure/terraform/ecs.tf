...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([
    {
      name  = "backend"
      image = "${local.account_id}.dkr.ecr.${local.region}.amazonaws.com/${var.project_name}-backend:latest"
...
resource "aws_ecs_service" "backend" {
  ...
  # Depend on HTTP listener (always exists)
  # HTTPS listener will be created first if domain_name is set due to redirect reference
